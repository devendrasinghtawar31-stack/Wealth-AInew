    ...
    const fetchUserGoalsStream = async () => {
        try {
            const [goalsResponse, financeResponse] = await Promise.all([
                ...
                API.get("/ai/advice").catch(() => ({ data: { totalIncomeCalculated: 0, totalExpenseCalculated: 0 } }))
            ]);
        ...
        <div style={{ marginTop: "20px", display: "flex", flexDirection: "column", gap: "30px", width: "100%" }}>
            <div>
                ...
                <p style={{ margin: 0, color: theme.colors.textMuted, fontSize: "13px" }}>
                    Set your financial targets and track how closely your real-time balance aligns with your goals.
                </p>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "25px" }}>
                {loading ? (
                    <div style={{ color: theme.colors.textMuted, fontSize: "13px" }}>Assembling target tracking blocks...</div>
                ) : goalsList.length === 0 ? (
                    <div style={{ color: theme.colors.textMuted, fontSize: "13px", padding: "20px", gridColumn: "1/-1" }}>
                        You haven't set any financial goals yet. Create a new target milestone to get started!
                    </div>
                ) : (
                    goalsList.map((goal) => {
                        ...
                        if (remaining <= 0) {
                            ...
                            timelineMessage = "Budget Alert: Current spending levels may delay your progress. Consider increasing your savings to stay on target."
                        } else {
                        ...
                        return (
                            <div key={goal._id} style={{
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
                                    <div style={{ fontSize: "12px", fontWeight: "600", color: "#FF9F1C", fontFamily: "monospace" }}>
                                        {timelineMessage}
                                    </div>
                                    ...
                                    <button 
                                        ...
                                        onMouseEnter={(e) => e.target.style.background = "rgba(255, 75, 75, 0.18)"}
                                        onMouseLeave={(e) => e.target.style.background = "rgba(255, 75, 75, 0.08)"}
                                    >
                                        Delete
                                    </button>
                                </div>
                                ...
                                <div style={{ width: "100%", height: "100px", background: "rgba(0,0,0,0.15)", borderRadius: "10px", padding: "8px", marginTop: "5px" }}>
                                    <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={chartData}>
                                            <XAxis dataKey="name" stroke={theme.colors.textMuted} fontSize={9} tickLine={false} />
                                            <Tooltip contentStyle={{ background: theme.colors.surface, border: `1px solid ${theme.colors.border}`, color: "#FFF", fontSize: "11px" }} />
                                            <Area type="monotone" dataKey="bachat" stroke="#FF6B35" fill="rgba(255, 107, 53, 0.12)" strokeWidth={2} />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                </div>
                            </div>
